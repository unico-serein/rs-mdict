...
use encoding_rs::{GB18030, UTF_16LE, BIG5};
use regex::Regex;
    ...
    match encoding {
        Encoding::Utf8 => {
            String::from_utf8(bytes.to_vec())
                .map_err(|e| MdictError::EncodingError(e.to_string()))
        }
        Encoding::Utf16Le => {
            let (result, _, had_errors) = UTF_16LE.decode(bytes);
            if had_errors {
                Err(MdictError::EncodingError("UTF-16LE decode error".to_string()))
            } else {
        ...
        Encoding::Gb18030 => {
            ...
            if had_errors {
                Err(MdictError::EncodingError("GB18030 decode error".to_string()))
            } else {
                Ok(result.into_owned())
            }
        }
...
pub fn parse_header(header_text: &str) -> Result<HashMap<String, String>> {
    ...
    for cap in re.captures_iter(header_text) {
        ...
        let value = cap.get(2).map(|m| unescape_entities(m.as_str())).unwrap_or_default();
