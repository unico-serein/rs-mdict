...
fn test_mdx_lookup_hello() {
    ...
    println!("Definition preview: {}...", &result.definition[..result.definition.len().min(500)]);
}
...
fn test_mdx_contains() {
    ...
    assert!(!mdx.contains("xyznonexistent123"), "Random word should not exist");
...
fn test_mdd_resource_info() {
    ...
    for key in keys.iter().take(10) {
        if let Some(info) = mdd.get_resource_info(key) {
            println!("  {} -> ext: {}, mime: {}", info.key, info.extension, info.mime_type);
        }
    }
}
...
fn test_mdd_contains() {
    ...
    assert!(!mdd.contains("\\nonexistent_resource_xyz.abc"), "Random resource should not exist");
...
fn test_mdx_associate() {
    ...
    println!("Found {} associated words in the same key block", associated.len());
    ...
    if !associated.is_empty() {
        ...
        assert!(associated.iter().all(|item| item.key_block_idx == block_idx),
            "All associated words should be in the same key block");
    }
}
...
fn test_mdx_fetch() {
    ...
    assert!(!result.definition.is_empty(), "Definition should not be empty");
}
...
fn test_mdx_keyword_list() {
    ...
    for item in keyword_list.iter().take(10) {
        println!("  {} (block: {}, start: {}, end: {})", 
            item.key_text, 
            item.key_block_idx, 
            item.record_start_offset,
            item.record_end_offset
        );
    }
...
fn test_mdx_fuzzy_search() {
    ...
    for fw in fuzzy_results.iter() {
        println!("  {} (edit distance: {})", fw.item.key_text, fw.edit_distance);
    }
    
    // Should find "apple" with edit distance 1
    assert!(fuzzy_results.iter().any(|fw| fw.item.key_text.to_lowercase() == "apple"),
